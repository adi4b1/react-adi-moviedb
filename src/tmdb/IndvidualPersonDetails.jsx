import React, { useContext, useEffect } from "react";
import { IndPersonDataContext } from "./usecontext/IndPersonData";
import { useParams } from "react-router-dom";
import { poster_parent_url } from "./MovieDetails";
import { useDispatch } from "react-redux";
import { increment } from "../redux/counterSlice";
import { useSelector } from "react-redux";

const IndvidualPersonDetails = () => {
  const { id } = useParams();
  const indId = Number(id);
  const dispatch = useDispatch();
  const { persondetails, getIndividualPersonData } =
    useContext(IndPersonDataContext);

  const likeHandler = () => {
    dispatch(increment());
  };

  const likeCount = useSelector((state) => state.counter.value);
  useEffect(() => {
    getIndividualPersonData(indId); // Pass indId directly
  }, [indId]);

  return (
    <div>
      {/* {indId} */}
      {/* <img src={persondetails.data.profile_path?`${poster_parent_url}${persondetails.data.profile_path}`:""} className="forImDim"alt={indpersonId} /> */}

      {/* <p onClick={likeHandler}>Like</p>
<div>Likes:{likeCount}</div> */}
      {persondetails.error && <p>getting error check network connection</p>}
      {persondetails.load && (
        <div className="actordetailspinner d-flex justify-content-center">
          <div className="spinner-border" role="status">
            <span className="visually-hidden">Loading...</span>
          </div>
        </div>
      )}

      {!persondetails.load && persondetails.data && (
        <div className="indPersonDetailsblock">
          <div className="firstBlock">
            <div className="forImg">
              {/* {indId} */}
              {persondetails.data.profile_path ? (
                <img
                  src={`${poster_parent_url}${persondetails.data.profile_path}`}
                  className="forImDim"
                  alt={persondetails.data.name || "Person"}
                />
              ) : (
                <img
                  src={`data:image/png;base64,`}
                  className="forImDimsec"
                  alt={persondetails.data.name || "Person"}
                />
              )}
              <div>
                {persondetails.data.birthday ? (
                  <p>🎂&nbsp;{persondetails.data.birthday}</p>
                ) : (
                  ""
                )}

                {persondetails.data.place_of_birth ? (
                  <p>🗓️&nbsp;{persondetails.data.place_of_birth}</p>
                ) : (
                  ""
                )}
                {/* {persondetails.data.birthday?(
                        <p>{persondetails.data.birthday}</p>
                    ):("")} */}
                {persondetails.data.deathday ? (
                  <p>{persondetails.data.deathday}</p>
                ) : (
                  ""
                )}
                {persondetails.data.known_for_department ? (
                  <p>✨&nbsp;{persondetails.data.known_for_department}</p>
                ) : (
                  ""
                )}

                {persondetails.data.also_known_as ? (
                  <div style={{ padding: "5px" }}>
                    <h6>Nicknames</h6>
                    {persondetails.data.also_known_as.map((item, index) => {
                      return (
                        <span key={index}>
                          {item}
                          {" , "}
                        </span>
                      );
                    })}
                  </div>
                ) : (
                  ""
                )}
              </div>
            </div>
            <div className="forExtraInfo">
              <h3 style={{ padding: "10px" }}>{persondetails.data.name}</h3>
              {persondetails.data.birthday ? (
                <p>{persondetails.data.biography}</p>
              ) : (
                ""
              )}
            </div>
          </div>
          <div></div>
        </div>
      )}
    </div>
  );
};

export default IndvidualPersonDetails;
