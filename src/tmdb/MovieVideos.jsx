  ...
  return (
    ...
    <div className='videosParent'>
      ...
      <div className='firstBlock'>
        <h2>Types</h2>
        <small><em>Please select the types</em></small>
        <select name="type" onChange={(e)=>changeHandler(e)}>
                {[...new Set(videos?.results?.map((i,index)=>i.type))]
                .map((type,index)=>{
        ...
        <div className='mainVideosBody'>   
        {
            ...
            videos?.results?.map((item,index)=>{
                ...
                <div className='videosDisplay' key={index}>
                    {/* {videoType} */}
                        <div onMouseEnter={()=>playVideo(index)} 
                            ...
                            key={item.key} lazy="loading" src={`https://www.youtube.com/embed/${item.key}?enablejsapi=1&mute=1`} 
                            className="video"frameBorder="0"></iframe>
                        </div>
                        <div className='forTextVideo'>
                            <h6 style={{fontSize:'14px'}}>{item.name}{item.official&&(
                                '✅'
                            )}</h6>
                            <small style={{fontSize:'12px'}}>⏱️ {new Date(item.published_at).toLocaleString('en-IN'
                                ,{
                                    day:'2-digit',
                                    month:'short',
                                    year:'numeric',
